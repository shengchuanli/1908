...
class HistoryCommand extends Command
{
    ...
    protected function configure()
    {
        ...
        $this
            ...
            ->setAliases(['hist'])
            ->setDefinition([
                new InputOption('shop',        's', InputOption::VALUE_REQUIRED, 'Show the given range of lines.'),
...
<return>>>> history --shop 0..10 --replay</return>
...
HELP
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($save = $input->getOption('save')) {
            ...
            if (!($input->getOption('shop') || $input->getOption('head') || $input->getOption('tail'))) {
                throw new \InvalidArgumentException('You must limit history via --head, --tail or --shop before replaying');
            }
    ...
    private function getHistorySlice($show, $head, $tail)
    {
        ...
        // don't shop the current `history` invocation
        \array_pop($history);
